...
/**
 * Superclass for tests which use Jackson functionality (such as de/serialization tests).
 */
public class JacksonTest {
    protected final ObjectMapper jackson;

    protected JacksonTest(Consumer<ObjectMapper> configurator) {
        this.jackson = new ObjectMapper();
        configurator.accept(jackson);
    }
    ...
    protected <T> void test(Class<T> typeClass, T original) {
        ObjectWriter writer = jackson.writerFor(typeClass);
        
        final String originalJson = assertDoesNotThrow(() -> writer.writeValueAsString(original));
        
        final T versionA = assertDoesNotThrow(() -> jackson.readValue(originalJson, typeClass));

        final String versionAJson = assertDoesNotThrow(() -> writer.writeValueAsString(versionA));

        final T versionB = assertDoesNotThrow(() -> jackson.readValue(versionAJson, typeClass));
